...
import time
import sys
sys.path.append('./core/face_detector')
...
class FaceDetector:
    ...
    def detect_face(self, img, threshold=0.7):
        outputs = self.__call__(img, threshold)

        bboxes = []
        for box_index in range(outputs.shape[0]):
            bbox = outputs[box_index]
